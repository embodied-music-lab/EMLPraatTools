...
<!-- APPEND-ONLY. Everything below the verbatim block is dated and
     chronological. A correction is a NEW entry saying what an earlier entry
     got wrong; nothing already written here is edited or deleted. This file
     never states current finding status -- that is the register's only job.
     See the entry of 18 August 2026. -->
...
*Newest last. Append; do not edit. Every entry carries a date, and an entry
that corrects an earlier one says which entry and what it got wrong, in a new
entry of its own.*

---

## 2026-08-18 — the register and the narrative are split, and nine findings are migrated

**This entry records a ruling from the verification session, and the work done
under it. It is the first entry in this file.**

### What this file is, and what it is not

Everything above the `Entries` heading is `audit/FINDINGS_INDEX.md` as it stood
at commit `b619683`, reproduced verbatim and never to be altered. It was the
audit's working document from 4 August 2026 onward and it is kept whole because
the audit record keeps its history: the repository's current-behaviour-only rule
scopes shipped files, not `audit/`. The newest thing that was ever true in it is
dated 8 August 2026. Several of its sentences were already false when it was
last committed, and this entry names three of them below.

From here down, this file is a lab notebook. It is **append-only** and
**chronological**. It **never states current status**. If you want to know
whether a finding is open, closed, unpinned, superseded or refuted, the answer
is not in this file and must never be looked for here — it is in
`audit/FINDINGS_MACHINE.json`, rendered for reading as `audit/FINDINGS_INDEX.md`.

### The ruling

`FINDINGS_MACHINE.json` is authoritative. It is the only status surface a
program checks, every piece of ledger machinery built this week was built on it,
and the generation direction only works one way: deriving structure from prose
is fragile, deriving prose from structure is a report generator. So:

1. **The nine findings that lived only in prose get rows.** D88, D97, D110,
   D123, D124, D125, D126, D134, D135. Statuses taken from the best-evidenced
   current source, with `fixedBy` and `pinnedBy` per the standing schema rules.
2. **New findings are filed in the register only.** Prose may discuss; only the
   register states.
3. **`FINDINGS_INDEX.md` keeps its name and becomes the generated register
   view.** Regenerated from the JSON by `validate/tools/gen_findings_index.py`,
   with a banner on line 1 saying so, and `--check` regenerates and diffs — the
   same pattern as the export-ignore agreement check. An index that is current
   and derived is what the name always promised.
4. **The narrative moves here**, append-only and chronological, opening with the
   old index's full content verbatim.
5. **Ten other dated documents are historical records**, true at their date and
   never updated, each carrying one mechanical banner line applied by
   `validate/tools/banner_historical.py`.
6. **The session task list is not a record and is never cited as one.**

### Why: the interleaving was the defect

One file was doing two jobs, and a stale sentence survived beside its own
correction because of it. The clearest case is D125. A status paragraph 14 lines
into the old index says "**D125 is NOT closed** — it is in flight". Its row, 290
lines further down, says "**LIVE 7 Aug**", and describes `v28` as pinning the gap
*as silence* so that it "fails the day anyone closes it". Somebody closed it.
`validate/v28_column_type_guard.R` says so in its own header — "This is that
day" — and replaced the two silence pins with the refusal verbatim at its lines
979-980. Nothing about the index was wrong at the moment each of those three
sentences was written. The document was wrong the day after, in a way no reader
could see, because a register row and a dated narrative aside look identical on
the page and only one of them is supposed to age.

The same shape, twice more, both found by measuring rather than by reading:

- **D135.** The old index row says "**LIVE 8 Aug**" and gives the repair as
  future work. `validate/v32_legend_geometry.R` records at its lines 1111-1150
  that its section 6 was written to pin D135 *open* and had to be rewritten
  before it was ever committed, because the placement work closed the finding a
  few hours after the fixture first measured it.
- **D88.** The old index row says the fix landed at 10 sites with "2 F0 sites
  left by design". `plugin_EML_StatsGraphs/dev/FIX_NOTES.md:137` records the
  propagation being completed the same day to all 16 sites, and the tree agrees:
  every `@emlComputeAxisRange` call in `graphs/` derives its granularity from
  `@emlComputeNiceStep`.

In each case the prose and the source disagreed and **the source won**. Each
migrated row says so in its own `measured` field, naming the source it was
settled from.

### What was measured, and what it cost

Two of the nine could not be closed, and the register now says so out loud
rather than inheriting the old index's word:

- **D124** (annotation block wrapping) is `fixed-unpinned`. The fix is real and
  present at `graphs/eml-annotation-procedures.praat:1225-1250`. Nothing holds
  it: no file under `validate/` asserts the block's *width* against the frame.
  `v29`'s nearest check bounds the block's *line count*, which is the vertical
  defect and not this one.
- **D126** (the harness recipe's `pkill -f` self-kill hazard) is
  `fixed-unpinned` for the same reason. Every `pkill`/`pgrep` invocation in the
  recipe and in the eight harness `run.sh` files is now the `-x` exact-name
  form, and nothing under `validate/` reads `GUI_HARNESS_RECIPE.md`, so a `-f`
  reintroduced tomorrow would be caught by nothing.

And one departs from the ruling's own word, deliberately:

- **D134.** The ruling calls it `superseded`, on the correct observation that
  the wizard now *calls* the shared normality rule rather than mirroring it —
  which the tree confirms at `scripts/eml-wizard.praat:2218-2249`, and which is
  a strictly stronger repair than the old index credits ("now mirrors the
  hierarchy branch for branch"). But `superseded` is not a synonym in this
  register. It is entailed by a `roadmap:` pointer, it means the work moved to a
  roadmap phase and *was never built*, and the schema checker forbids it from
  carrying either a fix or a pin. Work was built here and it is in the tree, so
  the honest literal is `fixed-unpinned` — which admits the fix and admits that
  no validator asserts the delegation. Filing it `superseded` would have
  required writing a `roadmap:` pointer at a phase that does not exist.

### The completeness lint, which is the piece that prevents recurrence

`validate/tools/check_findings_schema.py` gained an **ID census**. It walks the
prose of this repository, extracts every token matching the finding-ID grammar,
and fails if an ID appears in prose with no row in the register. "The register
lacks nine findings" is now a red check the day it becomes true, rather than
something an audit discovers weeks later. The grammar it matches, what it
deliberately does not match, and the two break-tests it was watched go red on,
are documented in that file's header.

### Suite

Run at the end of this work. The count and the commit hashes are in the session
