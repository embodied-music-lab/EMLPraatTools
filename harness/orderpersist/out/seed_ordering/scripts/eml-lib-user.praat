# ============================================================================
# eml-lib-user.praat -- GENERATED. Do not edit.
#
# EML Stats & Graphs -- Ian Howell -- Embodied Music Lab
# GPL-3.0-or-later
#
# One line loads the whole stack, from a script anywhere on this
# machine:
#
#     include ~/plugin_EML_StatsGraphs/scripts/eml-lib-user.praat
#
# WHEN THIS FILE IS WRITTEN. setup.praat runs at every Praat launch
# and rewrites this file whenever its contents would differ from what
# is on disk -- when the plugin moves, or when a Praat version puts
# its preferences folder somewhere else. A launch that would change
# nothing writes nothing. Anything edited into this file by hand is
# overwritten the next time the paths change.
#
# WHY IT IS GENERATED RATHER THAN SHIPPED. Praat's `include` is a
# parse-time text paste, and a relative path inside an included file
# resolves against the TOP-LEVEL script's folder -- the folder of
# whoever includes it, not this file's own. A barrel of relative
# lines therefore looks for the plugin inside the user's own folder.
# Only a file written by something that knows the installed location
# can carry paths that resolve, and setup.praat runs from it.
#
# The paths are home-relative: `include` accepts a leading ~, so this
# file is correct for any account on this machine.
# ============================================================================

include ~/plugin_EML_StatsGraphs/stats/eml-core-utilities.praat
include ~/plugin_EML_StatsGraphs/stats/eml-core-descriptive.praat
include ~/plugin_EML_StatsGraphs/stats/eml-extract.praat
include ~/plugin_EML_StatsGraphs/stats/eml-output.praat
include ~/plugin_EML_StatsGraphs/stats/eml-inferential.praat
include ~/plugin_EML_StatsGraphs/stats/eml-psychometrics.praat
include ~/plugin_EML_StatsGraphs/stats/eml-categorical.praat
include ~/plugin_EML_StatsGraphs/stats/eml-result-writer.praat
include ~/plugin_EML_StatsGraphs/stats/eml-record.praat
include ~/plugin_EML_StatsGraphs/graphs/eml-graph-procedures.praat
include ~/plugin_EML_StatsGraphs/graphs/eml-annotation-procedures.praat
include ~/plugin_EML_StatsGraphs/graphs/eml-draw-procedures.praat
include ~/plugin_EML_StatsGraphs/stats/eml-analysis.praat
include ~/plugin_EML_StatsGraphs/stats/eml-demo-tables.praat
