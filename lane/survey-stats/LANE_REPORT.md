...
## Round 4, 17 Aug (Ian's ruling via the verification relay):
## respondent influence on alpha

@emlAlphaInfluence added to eml-psychometrics.praat: leave-one-out
jackknife over respondents, the other axis from alpha-if-deleted. Same
input shape and listwise deletion as the alpha kernel; outputs the full
complete-case alpha, every leave-one-out alpha and its delta, and the
largest absolute delta — always reported under ORIGINAL row numbers via
.rowIndex#, so exclusions above a respondent never shift who the report
names. The kernel reports and never judges: no thresholds, no outlier
labels.

Implementation: each leave-one-out alpha comes from downdating the full
per-item and total-score sums (one vector operation per respondent via
row#), not from rebuilding submatrices; the respondent loop is the
jackknife itself. Floor: k >= 2 and n >= 3 complete respondents,
refused with a message below that. A leave-one-out subset with zero
total variance yields undefined for that entry, disclosed not refused.

Validation to the same standard: v93 (47 checks) drives Praat live on
four fixtures — the clean alpha matrix, a planted-deviant matrix
(dominance engineered and pinned: the deviant's delta must exceed the
runner-up by half again), the mapping fixture with missing cells BEFORE
the deviant (dominant delta must come back as original row 17, and the
validator also asserts the surviving index differs, so the pin cannot
pass trivially), and an n = 3 edge — against a base-R LOO oracle built
on v90's covariance-matrix alpha. No packages: psych has no equivalent
of this jackknife, so this validator is base-R-only by nature. The
alphaFull = @emlCronbachAlpha.alpha equality is pinned at 1e-12 in the
same probe. Negative control: seeded wrong baseline (deltas against
0.99 x alphaFull); differs-check green, red demonstration exits 1. Dev
tests grew to 37 (from 20) with all new literals verified by the
extended companion; the oracle dump now records 101 values. Kernel
