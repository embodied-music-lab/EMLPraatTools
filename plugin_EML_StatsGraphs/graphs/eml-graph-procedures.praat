...
# @emlReadLtasBins, @emlBuildFrameTable and @emlToTable MOVED to
# stats/eml-extract.praat on 9 Sep 2026 -- the shared low-level layer both
# stats and graphs load before either module's own files (eml-lib.praat ->
# eml-lib-stats.praat -> stats/eml-extract.praat, then eml-lib-graphs.praat).
# @emlToTable's own real dependencies (@emlExtractPitchValues,
# @emlExtractIntensityFrames, @emlExtractHarmonicityFrames) already lived in
# that file, so landing it there puts it after its own dependencies and
# before stats/eml-output.praat, the nearest position still upstream of
# every caller -- the two stats-side doors (@emlWrapperInit,
# @emlDescribeCoerceSelection) reach it now too.
#
# The call site immediately below (inside @emlConvertForGraph) and both
# call sites in graphs/eml-graphs-form.praat (:1115, :1127) needed zero
# syntax changes: the barrel always resolves @emlToTable: ... from
# stats/eml-extract.praat before any graphs file executes. Likewise
# @emlDrawLTAS's two direct calls to @emlReadLtasBins in
# graphs/eml-draw-procedures.praat (:1174, :1326).
#
# No forwarding procedure is redeclared at this site on purpose: Praat
# procedure names are one global flat namespace assembled by textual paste
# ("a second definition ... silently replaces the first" -- see this
# barrel's own header), and graphs always loads after stats, so a second
# `procedure emlToTable` block left here would be the LAST-parsed
# definition and would silently shadow the real one in stats/eml-extract.praat
# for every caller in the plugin, including the two new stats-side doors --
# and it could not "delegate" under its own name without becoming infinite
# self-recursion. The barrel's fixed load order is the forwarder: every
# consumer of this file already resolves @emlToTable/@emlReadLtasBins from
# stats/eml-extract.praat, confirmed for every call site above.
#
# @emlCleanConvertedTable did NOT move -- see its own header just below --
# it stays here, and @emlToTable (from its new home) still reaches it fine.
# See stats/eml-extract.praat and REGISTRY.tsv.
