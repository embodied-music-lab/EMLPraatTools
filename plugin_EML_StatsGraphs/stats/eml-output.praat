...
# ----------------------------------------------------------------------------
# @emlReportGroupOrderLine
# Punch item 2.2 (ordering half): one DISCLOSURE line on every grouped
# comparison report stating which group order is in force, so a reader
# knows the order was a choice and which choice it was. Wording is
# LANGUAGE_BATCH_2026-08-25.md item 10, verbatim, and prints on every path
# regardless of the explanations toggle.
#
# .groupList$ is built BY THE CALLER from whatever group-name array the
# analysis already resolved (emlPairwiseT.groupName$[], emlOneWayAnova.
# groupLabel$[], etc.) -- never re-derived here, so the order named is
# always the order the numbers above it were actually computed in.
# emlGroupSortAlphabetical is the one flag every door funnels the user's
# choice through (see @emlCountGroups), so reading it here rather than a
# door-local copy is what keeps menu, wizard and graph doors in agreement.
#
# Arguments:
