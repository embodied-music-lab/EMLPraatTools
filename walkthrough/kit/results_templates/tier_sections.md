# Tier sections — one register for SUMMARY.md's three tiers

Added 28 August 2026, alongside the tier split (Ian's ruling,
...
Keyed by tier name (`options`, `sweep`, `nist`), matching matrix.tsv's own
`tier` column.
...
## sweep
A different question from the options tier, asked over a designed grid of
...
homoscedastic and 10:1 heteroscedastic. Where the options tier asks whether a
printed report is right, this tier asks whether the ANOVA, Tukey and
...
## nist
A question the other two tiers cannot ask, because both of them compare the
...
tier -- `compare.R` compares the plugin's own column against those constants
